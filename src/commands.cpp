...
void handle_wifi(String args)
{
    if (args == "scan")
    {
        ...
        if (numFound > 0)
        {
            ...
            {
                uint16_t signalColor = (WiFi.RSSI(i) > -60) ? TFT_GREEN : (WiFi.RSSI(i) > -80) ? TFT_ORANGE
                                                                                               : TFT_RED;
...
void handle_sms(String args)
{
    ...
    else if (args.startsWith("read"))
    {
        ...
        if (msg == "ERROR")
            terminal.println("Error when reading", TFT_RED);
        else if (msg == "NONE")
            terminal.println("No new messages", TFT_YELLOW);
        else
        {
...
void handle_modem(String args)
{
    if (args.startsWith("init"))
    {
        if (modem.begin())
        {
            terminal.println("Init Success!", TFT_GREEN);
        }
        else
        {
            terminal.println("Init Error!", TFT_RED);
        }
    }
    else if (args.startsWith("send"))
    {
        modem.sendAT(args.substring(5).c_str());
        terminal.println(modem.readResponse(5000));
    }
}
